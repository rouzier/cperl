...
/* Called by cv_type_set and newATTRSUB for an extern sub,
   to set its private return type. */
PADNAMELIST *
Perl_cv_clone_padnames(pTHX_ CV *cv, PADNAMELIST *pnl)
{
    PADNAME **pnp;
    PERL_ARGS_ASSERT_CV_CLONE_PADNAMES;

    pnp = PadnamelistARRAY(pnl);
    if (pnp) {
        /* padnamelist_dup(pnl, CLONE_param) */
        ...
        DEBUG_X(cv_dump(cv, "cv_clone_padnames"));
        DEBUG_Xv(padlist_dump(CvPADLIST(cv)));
    }
    return pnl;
}
